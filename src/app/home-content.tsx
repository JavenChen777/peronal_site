...
import { sortTools, ALL_CATEGORIES, getCategoryColor } from '@/lib/utils';
...
const HOT_TAG_CATEGORIES: ToolCategory[] = ['developer', 'design', 'data', 'media', 'productivity'];
    ...
    <div>
      ...
      <section className="border-border to-background relative overflow-hidden border-b bg-gradient-to-b from-brand-900/20 py-20 dark:from-brand-900/30">
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {HOT_TAG_CATEGORIES.map((category) => {
      ...
      <section className="border-border bg-muted/30 border-b">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-center gap-8 px-4 py-5 text-center sm:px-6">
          <div>
            ...
            <div className="text-muted-foreground text-xs">{t.home.stats.totalTools}</div>
          </div>
          <div className="bg-border h-8 w-px" aria-hidden="true" />
          <div>
            <div className="text-foreground text-2xl font-bold">{totalCategories}</div>
            <div className="text-muted-foreground text-xs">{t.home.stats.categories}</div>
          </div>
          <div className="bg-border h-8 w-px" aria-hidden="true" />
          <div>
            <div className="text-foreground text-2xl font-bold">100%</div>
            <div className="text-muted-foreground text-xs">{t.home.stats.openSource}</div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h2 className="text-foreground text-2xl font-bold">{t.home.featured.title}</h2>
            <p className="text-muted-foreground mt-1 text-sm">{t.home.featured.subtitle}</p>
          </div>
          <Link
            href="/tools"
      ...
      <section className="border-border bg-muted/20 border-t">
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
          <h2 className="text-foreground mb-6 text-xl font-bold">{t.home.browseByCategory}</h2>
